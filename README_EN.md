...
**Java版openClaw** — 用多种技术框架编写，支持多模型、多通道、多 Agent 协同的开源软件
...
[![GitHub](https://img.shields.io/badge/GitHub-seifly/jcalw-black?logo=github)](https://github.com/seifly/jclaw)
[![Gitee](https://img.shields.io/badge/Gitee-sablog/jclaw-red?logo=gitee)](https://gitee.com/sablog/jclaw)
...
- **💬 多通道消息** — 同时连接 微信、企业微信、飞书、钉钉、QQ、MaixCam 、Telegram、Discord、WhatsApp等平台
- **🤝 多 Agent 框架版本** — 3 种框架（springAI, AgentScope, 自定义），详见[GitHub分支](https://github.com/seifly/jclaw)
...
git clone https://gitee.com/sablog/jclaw.git
cd jclaw
...
> **注意**：微信通道无需配置API密钥，启动后通过Web界面扫码登录即可。
...
##### 2. 消息通道管理
监控 Telegram、Discord、飞书、钉钉、微信等各平台消息通道的连接状态。

![通道管理](docs/.img/img_1.png)

**微信通道特别说明：**
- 微信通道采用扫码登录方式，无需配置API密钥
- 首次使用时，点击“设置”按钮会显示登录二维码
- 使用微信扫描二维码完成登录后，系统会自动保存登录状态
- 重启应用后会自动恢复之前的登录状态，无需重新扫码
- 如遇到登录问题，可点击“刷新二维码”重新生成
...
| 微信 | `channels.wechat` | 扫码登录（iLink SDK） |
...
微信通道基于 iLink SDK 实现，具有以下特色功能：

- **扫码登录**：无需复杂配置，通过扫描二维码即可登录
- **会话保持**：自动保存登录状态，重启后自动恢复连接
- **消息去重**：智能处理重复消息，避免重复响应
- **断线重连**：网络异常时自动尝试重新连接
- **二维码超时处理**：提供清晰的超时提示和重新生成机制
...
**发送消息**：
- Agent 可以直接发送 QQ 消息到指定用户
- 支持私聊和群聊 @ 消息
- 自动管理访问令牌刷新

**接收消息**：
- 通过 Webhook 端点接收外部网关转发的消息
- Webhook 地址：`POST /api/channels/qq/webhook`
- 需要部署 QQ 机器人网关服务（如 go-cqhttp、Lagrange.Core 等）
- 详细的网关集成指南请查看 [docs/qq-gateway-integration.md](docs/qq-gateway-integration.md)
...
curl -X POST http://localhost:18790/api/channels/qq/webhook \
  ...
  -d '{"id":"test_001","content":"你好","author":{"id":"123456"}}'
```
...
- [miniClaw](https://github.com/memglongdeqiangse/mini-claw) — 提供多渠道接入、工具调用、定时任务调度和会话持久化，开箱即用。
